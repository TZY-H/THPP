...
int main()
{
    ...
    for (int32_t goal_index = 0; goal_index < 4; goal_index++)
    {
        ...
        t0 = utime_ns();
        bool tf = cemap.THPPoptimalPlanner(task, task.HomotopyPolyIndex_Init, HomotopyPolyIndex_Goal,
                                           task.Init, task.Goal, task.minPath);
        std::cout << " time: " << (utime_ns() - t0) << std::endl;
        if (tf)
        {
            image = cemap.BIdmap.clone();
            // drawBIpath(task.minPath, image, cv::Scalar(0, 0, 255));
    ...
    for (int32_t goal_index = 0; goal_index < 4; goal_index++)
    {
        ...
        {
            cost += cemap.THPPoptimalPlanner(task, task.HomotopyPolyIndex_Init, HomotopyPolyIndex_Goal,
                                             task.Init, task.Goal, task.minPath);
        }
        std::cout << " time: " << (utime_ns() - t0) / testNN << std::endl;
        std::cout << " cost: " << cost / testNN << std::endl;
    }
