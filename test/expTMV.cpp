...
int main()
{
    ...
    task.Init = testTask.init;
    int32_t initpoly = cemap.BIimap.at<uint16_t>(task.Init.y, task.Init.x);
    task.HomotopyPolyIndex_Init = (testTask.HomotopyIndex << 16) | initpoly;
    ...
    t0 = utime_ns();
    bool tf = cemap.TMVoptimalPlannerViolent(task, task.HomotopyPolyIndex_Init,
                                             testTask.init, testTask.goals, minpath);
    ...
    for (int32_t kk = 0; kk < testNN; kk++)
        cemap.TMVoptimalPlanner(task, task.HomotopyPolyIndex_Init,
                                testTask.init, testTask.goals, minpath);
    t1 = utime_ns();
    std::cout << "TMVoptimalPlanner time: " << (t1 - t0) / testNN << std::endl;

    double cost;
    t0 = utime_ns();
    for (int32_t kk = 0; kk < testNN; kk++)
        cost = cemap.TMVoptimalPlannerViolent(task, task.HomotopyPolyIndex_Init,
                                       testTask.init, testTask.goals, minpath);
    t1 = utime_ns();
    std::cout << "TMVoptimalPlannerViolent time: " << (t1 - t0) / testNN << std::endl;
